"""Grocery supervisor — Sprint 2 production implementation."""
...
async def handle_mock_task(ctx: Context, sender: str, task: MockDomainTask) -> None:
    ...
    if not _MOCK_FALLBACK:
        ctx.logger.warning(
            "[DEPRECATED] MockDomainTask on grocery supervisor. "
    ...
    forwarded = task.copy(
        update={"metadata": {**(task.metadata or {}), "grocery_supervisor": "forwarded"}}
    )
    ctx.logger.info(
        "[ROUTE] MockDomainTask request_id=%s -> grocery-worker …%s",
        task.request_id, _short(GROCERY_WORKER_ADDRESS),
    )
...
async def handle_detection_request(
    ...
    ctx.logger.info(
        ...
        "context_keys=%s -> grocery-worker …%s",
        msg.pass_id, msg.patient_id, _DOMAIN, sorted(domain_context.keys()),
        _short(GROCERY_WORKER_ADDRESS),
    )
    await ctx.send(GROCERY_WORKER_ADDRESS, detection_for_worker)
    ...
    if validation_errors:
        ...
        if retry_count < _MAX_DETECTION_RETRY:
            if retry_msg is not None:
                ...
                ctx.logger.warning(
                    "[CORRECTION-REQUEST] pass_id=%s domain=%s attempt=%d -> grocery-worker …%s",
                    result.pass_id, _DOMAIN, next_retry, _short(GROCERY_WORKER_ADDRESS),
                )
                await ctx.send(GROCERY_WORKER_ADDRESS, resend_msg)
                return
...
async def handle_question_request(
    ...
    ctx.logger.info(
        "[ROUTE] QuestionTask action_id=%s patient_id=%s patient_data_len=%d -> grocery-worker …%s",
        msg.action_id, msg.patient_id, len(patient_context), _short(GROCERY_WORKER_ADDRESS),
    )
    await ctx.send(GROCERY_WORKER_ADDRESS, task)
