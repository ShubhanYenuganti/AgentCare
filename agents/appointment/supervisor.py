"""Appointment supervisor — Sprint 2 production implementation."""
...
async def handle_mock_task(ctx: Context, sender: str, task: MockDomainTask) -> None:
    ...
    if not _MOCK_FALLBACK:
        ctx.logger.warning(
            "[DEPRECATED] MockDomainTask on appointment supervisor. "
    ...
    forwarded = task.copy(
        update={"metadata": {**(task.metadata or {}), "appointment_supervisor": "forwarded"}}
    )
    ctx.logger.info(
        "[ROUTE] MockDomainTask request_id=%s -> appointment-worker …%s",
        task.request_id, _short(APPOINTMENT_WORKER_ADDRESS),
    )
...
async def handle_detection_request(
    ...
    if msg.updated_domain not in (None, _DOMAIN):
        ctx.logger.warning(
    ...
    ctx.logger.info(
        ...
        "context_keys=%s -> appointment-worker …%s",
        msg.pass_id, msg.patient_id, _DOMAIN, sorted(domain_context.keys()),
        _short(APPOINTMENT_WORKER_ADDRESS),
    )
    await ctx.send(APPOINTMENT_WORKER_ADDRESS, detection_for_worker)
    ...
    if validation_errors:
        ...
        if retry_count < _MAX_DETECTION_RETRY:
            if retry_msg is not None:
                ...
                ctx.logger.warning(
                    "[CORRECTION-REQUEST] pass_id=%s domain=%s attempt=%d -> appointment-worker …%s",
                    result.pass_id, _DOMAIN, next_retry, _short(APPOINTMENT_WORKER_ADDRESS),
                )
                await ctx.send(APPOINTMENT_WORKER_ADDRESS, resend_msg)
                return
...
async def handle_modification_request(
    ...
    ctx.logger.info(
        "[ROUTE] ModificationTask action_id=%s patient_id=%s -> appointment-worker …%s",
        msg.action_id, msg.patient_id, _short(APPOINTMENT_WORKER_ADDRESS),
    )
    await ctx.send(APPOINTMENT_WORKER_ADDRESS, task)
...
async def handle_question_request(
    ...
    ctx.logger.info(
        "[ROUTE] QuestionTask action_id=%s patient_id=%s patient_data_len=%d -> appointment-worker …%s",
        msg.action_id, msg.patient_id, len(patient_context), _short(APPOINTMENT_WORKER_ADDRESS),
    )
    await ctx.send(APPOINTMENT_WORKER_ADDRESS, task)
